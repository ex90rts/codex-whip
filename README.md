...
需要 Xcode 16 Command Line Tools 或更高版本（Apple Swift 6.0+）。项目最低部署版本为 macOS 13，因此可在 macOS 15 上开发和运行。构建脚本会把编译缓存保存在项目内部，以兼容受限环境：
...
可用以下命令编译 macOS 15 的 Apple Silicon、Intel 目标，并在当前 Mac 上运行内置自检：

```bash
zsh scripts/check-macos15.sh
```

应用编译只需要 Command Line Tools。运行 `swift test` 还需要完整 Xcode 提供的测试运行时，并将 `xcode-select` 指向 Xcode 的 Developer 目录。
...
3. 应用会依次读取 Accessibility 宠物元素、WindowServer 窗口元数据和 Electron 保存的浮窗位置。
...
- 自动定位优先读取 Accessibility 中的宠物元素；未授权时不会弹出权限请求，而是继续读取窗口元数据。
- WindowServer 候选窗口按已知尺寸和层级筛选；旧版透明浮窗会按 Codex 的实际内部布局换算宠物位置，最后再尝试 Electron 持久化的 `electron-avatar-overlay-bounds`。
